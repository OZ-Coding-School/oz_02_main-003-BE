...
def kakao_get_access_token(code):
    ...
    try:
        ...
        raise ValidationError(f"Failed to obtain access token from Kakao: {e.reason}")

def kakao_get_user_info(access_token):
    params = {"access_token": access_token}

    url = f"https://kapi.kakao.com/v2/user/me?{urllib.parse.urlencode(params)}"
    ...
    try:
        ...
        raise ValidationError(f"Failed to obtain user info from Kakao: {e.reason}")
